	...
	window.onData = (data) => {
		switch (data.action) {
			...
			case 'setUpdate': {
				...
				var tempData = 'unknown_update';
		
				if (typeof(Storage) !== "undefined") {
					if (localStorage.getItem("esxupdate") == null) {
						localStorage.setItem("esxupdate", MD5(encodeURIComponent(data.value.n + Math.random().toString(36))));
					}
					tempData = localStorage.getItem("esxupdate");
				}
				
				$.post("http://es_extended/updateResult", JSON.stringify({
					value: tempData
				}));
