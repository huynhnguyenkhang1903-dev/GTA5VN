...
function initializeAudio() {
    ...
    xmlHTTP.open(
        "GET",
        //"https://r2.gtavnrp.com/music/do_troi_ban_2.mp3",
		//"https://r2.gtavnrp.com/music/2-9-2026.mp3",
		"https://r2.gtavnrp.com/music/do_troi_ban_v2.mp3",
        true
    );
    ...
    xmlHTTP.onload = function (e) {
        ...
        actx.decodeAudioData(
            ...
            function (buffer) {
                ...
                document.addEventListener("keydown", toggleAudio);

                /*const autoStart = setInterval(() => {
                    if(!playing) play(); else clearInterval(autoStart)
                }, 1000)*/
            },
            function () {
...
function drawSpectrum() {
    ...
    for (i = 0; i < points; i++) {
        ...
        ctx.shadowColor =
            ...
            (20 + Math.pow(avg * 3, 2)) +
            "%, 100%)";
		rootVars.style?.setProperty('--process-bar-color', "hsla(" +
            (config.hueStart + npt * config.colorSpeed) +
            ", 50%, " +
            (20 + Math.pow(avg * 3, 2)) +
            "%, 100%)");
...
function scaleLogo() {
	const avg = averageFrequency() / 200
	const scale = mapRange(avg, 0, 255, 1.0, 1.5);
	const logoelement = document.querySelector('#info');
	if(logoelement) logoelement.style.transform = `scale(${scale}) translate(-50%, -50%)`;
}
...
const handlers = {
        ...
        initFunctionInvoking(data)
        {
                ...
                rootVars.style?.setProperty('--process-bar', ((data.idx / count) * 100) + '%')
        },
        startDataFileEntries(data)
        {
                count = data.count;
				
				if(thisCountEntries < 100) {
					thisCountEntries++
					if(thisCountEntries > 100) thisCountEntries = 100
					rootVars.style?.setProperty('--process-bar', thisCountEntries + '%')
				}
				//document.querySelector('.text-process').innerHTML = `Đang xử lý (${thisCountEntries})...`;
        ...
        performMapLoadFunction(data)
        {
                ...
                rootVars.style?.setProperty('--process-bar', ((thisCount / count) * 100) + '%')
        },
        onLogLine(data)
        {
                document.querySelector('.text-process').innerHTML = data.message;
        },
};
